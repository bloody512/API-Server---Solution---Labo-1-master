    ...
    getAll(params = null) {
        ...
        if (params) {
            ...
            Object.keys(params).forEach(function (paramName) {
                ...
                {
                    for(let index = 0;index < model.validator.fields.length; index++)
                    {
                        if(paramName == model.validator.fields[index].name)
                        {
                            let keyValue = params[paramName];
                            searchKeys.push({key : paramName, cond : keyValue});
                        }
                    }
                }
            });
            ...
            for (var object in objectsList) {
                ...
                for (let searchKey of searchKeys) {
                    for(let index = 0;index < model.validator.fields.length; index++)
                    {
                        if(searchKey.key == model.validator.fields[index].name)
                        {
                            if(!this.valueMatch(objectsList[object][searchKey.key], searchKey.cond))
                            {
                                matchFilters = false;
                            }
                        }
                ...
                if(matchFilters)
                {
                    filteredAndSortedObjects.push(objectsList[object]);
                }
            }
            ...
            for(let sortKey of sortKeys)
            {
                for(let index = 0;index < model.validator.fields.length; index++)
                    {
                        let KeyName = model.validator.fields[index].name;
                        if(sortKey.key.toLowerCase() == KeyName.toLowerCase())
                        {
                            if(sortKey.asc)
                            {
                                filteredAndSortedObjects.sort((a, b) => a[KeyName] > b[KeyName] ? 1 : -1)
                            }
                            else
                            {
                                filteredAndSortedObjects.sort((a, b) => a[KeyName] < b[KeyName] ? 1 : -1)
